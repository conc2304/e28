 ...
 * @param {*} guess
 */
let handleGuess = (guess, wordToGuess, lettersGuessed, lettersToGuess, strikesLeft) => {
  ...
  if (guess.length > 1) {
    ...
    if (indicesOfGuess.length === 0) {
      strikesLeft = handleStrike(strikesLeft);
      if (strikesLeft === 0) {
        gameOver = handleGameOver('fail');
      }
    } else {
  ...
  */
 let validateWordCompletion = (wordToGuess, wordToGuessArr) => {

  let guessedStr = '';
  for (let letter of wordToGuessArr) {
    guessedStr += letter;
  }
...
let populateCorrectlyGuessedLetter = (letter, indices) => {
  for (let index of indices) {
    console.log(vm);
    vm.$set(vm.wordToGuessArr, index,letter);
  }
...
let GuessForm = {
  ...
  template: `<form @submit.prevent="handleGuess(guess, wordToGuess, lettersGuessed, wordToGuessArr, strikesLeft)" id="take-a-guess">
